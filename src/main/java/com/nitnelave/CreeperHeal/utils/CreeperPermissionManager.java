...
public class CreeperPermissionManager {
   ...
   * @param warning Whether this is a warning permission (i.e. ops shouldn't have it by default).
   * @param nodes The permissions to check.
  ...
  public static boolean checkPermissions(Player player, boolean warning, String... nodes) {
    if (!warning && player.isOp()) return true;
    if (!warning && player.hasPermission("CreeperHeal.*")) return true;
    for (String node : nodes) if (player.hasPermission("CreeperHeal." + node)) return true;
