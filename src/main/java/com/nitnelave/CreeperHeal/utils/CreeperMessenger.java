...
public abstract class CreeperMessenger {
  ...
  private static void load() {
    ...
    try {
      if (!messageFile.exists()) FileUtils.copyJarConfig(messageFile);
  ...
  /**
   * Create a formatted string ready for sending to a player or to the console. All the instances of
   * formatting keywords are replaced by the values provided if specified.
   *
   * @param type The message type, corresponding to the property key in the file.
   * @param values Values to replace the formatting keywords.
  ...
  public static String processMessage(String type, String... values) {
    ...
    try {
      for (int i = 0; i < variables.length; i++)
        if (values[i] != null) message = message.replaceAll("\\{" + variables[i] + "}", values[i]);
    } catch (NullPointerException e) {
  ...
  /**
   * Get the message that should be sent to a player/console after an event, as a formatted string.
   *
   * @param cause The reason for the message.
   * @param offender The name of the offender.
   * @param world The world in which the offense happened.
   * @param blocked Whether the action was blocked.
   * @param data The data describing the offense.
   * @param player Whether the message is meant for a player or for the admin.
  ...
  private static String getMessage(
    ...
    switch (cause) {
      ...
      case FIRE:
        message =
            processMessage(prefix + "flint-and-steel" + suffix, world, offender, null, null, null);
        break;
      case TNT:
      ...
      case BLACKLIST:
        message =
            processMessage(prefix + "place-blacklist" + suffix, world, offender, null, null, data);
        break;
      case SPAWN_EGG:
  ...
  private static void populateWarnList() {
    warnList.clear();
    for (Player p : Bukkit.getServer().getOnlinePlayers()) registerPlayer(new CreeperPlayer(p));
  }
   ...
   *
   * @param cause The reason for the message.
   * @param offender The offending player.
   * @param blocked Whether the action was blocked.
   * @param material Some info about the message.
   */
  public static void warn(WarningCause cause, Player offender, boolean blocked, String material) {
    String message =
        CreeperMessenger.getMessage(
            cause, offender.getName(), offender.getWorld().getName(), blocked, material, false);
    ...
    offender.sendMessage(
        CreeperMessenger.getMessage(
            cause, offender.getName(), offender.getWorld().getName(), blocked, material, true));
    for (CreeperPlayer cp : warnList) cp.warnPlayer(cp.getPlayer(), cause, message);
  }

  /**
   * Remove a player from the list of players that are warned of an infraction.
   *
  ...
  public static void registerPlayer(CreeperPlayer player) {
    if (player.hasWarnings()) warnList.add(player);
  }
}
