...
/**
 * 2-D map, allowing for fast neighbor search. The map is divided in zones, and in each zone there
 * is a list of Location.
 *
 * @param <T> The type of the data stored for each point.
 * @author nitnelave
 */
public abstract class NeighborFinder<T> {
  ...
  /**
   * Remove an element from the map. It must be in the zone at the x, y coordinate.
   *
   * @param el The element to remove.
   * @param x The x coordinate for the zone.
   * @param y The y coordinate for the zone.
   */
  public void removeElement(T el, double x, double y) {
    ...
    if (list == null) return;
    list.remove(el);
    if (list.isEmpty()) map.remove(p);
  }

  /**
   * Get whether the location is "close" to an element. The actual definition of "close" is in the
   * overridden methods of the children classes.
   *
   * @param loc The point to check for neighbors.
  ...
  /**
   * Get a neighbor of the location provided. It is not guaranteed to be the closest, but it is an
   * element which satisfies the condition of a neighbor.
   *
   * @param loc The location to check for neighbors.
