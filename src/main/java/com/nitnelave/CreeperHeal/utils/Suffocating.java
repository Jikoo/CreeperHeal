...
/**
 * Utility class to check if a player is suffocating, and teleport him to safety.
 *
 * @author nitnelave
 */
public abstract class Suffocating {
  ...
  /**
   * Check the living entities around the location. If any is suffocating, teleport it to a safe
   * location.
   *
   * @param loc The location around which to check.
   */
  ...
  /**
   * Check the players and other animals (except in lightweight mode) to see if they were trapped by
   * the explosion's replacement.
   *
  ...
  public static void checkPlayerExplosion(Location loc, double radius) {
    ...
    else entityList = loc.getWorld().getPlayers();
  ...
  private static Location check_player_suffocate(LivingEntity en) {
    ...
    if (CreeperBlock.isSolid(loc.getBlock())
      ...
      for (int k = 1; k + loc.getBlockY() < loc.getWorld().getMaxHeight(); k++) {
        ...
        if (check_free(l)) return l;

        l.add(k, -k, 0);
        if (check_free_horizontal(l)) return l;
