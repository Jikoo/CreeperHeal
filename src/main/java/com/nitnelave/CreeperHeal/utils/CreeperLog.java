...
public abstract class CreeperLog {
  ...
  static {
    ...
    if (!warningLogFile.exists()) FileUtils.createNewFile(warningLogFile);
  ...
  /**
   * Display an information message to the console, if the verbosity is high enough, i.e. if level
   * is lower or equals to the log level.
   *
   * @param msg The message to be logged.
   * @param level The corresponding verbosity of the message.
   */
  public static void logInfo(String msg, int level) {
    if (logLevel == -42) logLevel = CreeperConfig.getInt(CfgVal.LOG_LEVEL);
    if (level <= logLevel) {
  ...
  /**
   * Display the type and the location of a block in a formatted way. If force is true, then it is a
   * warning (as part of a warning message). Otherwise it is a debug message.
   *
   * @param block The block whose information is displayed.
   * @param force Whether it is a warning or a debug message.
   */
