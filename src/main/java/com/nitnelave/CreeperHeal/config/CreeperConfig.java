...
public abstract class CreeperConfig {
  ...
  private static void fillMaps() {
    ...
    for (CfgVal v : CfgVal.values())
      ...
      else if (v == CfgVal.ALIAS) alias = new StringConfigValue(v, getFile(v));
      else if (v == CfgVal.SOUND_NAME) {
        soundName = new StringConfigValue(v, getFile(v));
        loadSound();
      } else CreeperLog.warning("Unknown config value : " + v);
  }
   ...
   *
   * @param val The config key.
   * @return The boolean value.
   */
  public static boolean getBool(CfgVal val) {
    ConfigValue<Boolean> v = booleans.get(val.getKey());
    if (v == null) throw new NullPointerException("Missing config value : " + val.getKey());
    return v.getValue();
  }
  ...
  public static void setBool(CfgVal val, boolean value) {
    ConfigValue<Boolean> v = booleans.get(val.getKey());
    if (v == null) throw new NullPointerException("Unknown config key path : " + val.getKey());
    v.setValue(value);
  }
  ...
  private static WorldConfig loadWorld(String name) {
    ...
    if (w.hasGriefProtection()) CreeperHeal.registerGriefEvents();
    return w;
  }
  ...
  public static void write() {
    if (!CONFIG_FILE.exists() && !FileUtils.createNewFile(CONFIG_FILE)
        || !ADVANCED_FILE.exists() && !FileUtils.createNewFile(ADVANCED_FILE)) return;

    for (ConfigValue<Boolean> v : booleans.values()) v.write();

    for (ConfigValue<Integer> v : integers.values()) v.write();
    ...
    try {
      for (WorldConfig w : world_config.values()) w.save();
  ...
  /**
   * Load a world configuration file into memory the first time, and return the configuration.
   *
  ...
  protected static void remove(String key, boolean isAdvanced) {
    if (isAdvanced) advanced.set(key, null);
    else config.set(key, null);
  }
