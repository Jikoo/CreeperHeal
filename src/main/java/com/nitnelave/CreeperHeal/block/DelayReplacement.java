...
/**
 * This class is a task to replace a CreeperBlock later. If the block cannot be safely replaced,
 * then the replacement is postponed. After a numberof tries, the block is dropped to the ground.
 *
 * @author nitnelave
 */
public class DelayReplacement implements Runnable {
   ...
   *
   * @param replaceable The block to be replaced.
   * @param replaced The number of times a replacement has already been attempted.
  ...
  public void run() {
    if (counter < REPLACEMENT_THRESHOLD) {
      if ((blockState instanceof Attachable
              && blockState
                      .getBlock()
                      .getRelative(((Attachable) blockState).getAttachedFace())
                      .getType()
                  == Material.AIR)
          || blockState.getBlock().getRelative(BlockFace.DOWN).getType() == Material.AIR) counter++;
      else {
        CHBlockHealEvent event = new CHBlockHealEvent(blockState, true, reason);
        Bukkit.getPluginManager().callEvent(event);
        if (!event.isCancelled()) blockState.replace(event.shouldDrop());
