...
public abstract class CreeperMultiblock extends CreeperBlock {
  ...
  protected boolean checkForDrop() {
    if (checkForDependentDrop(getBlock())) return true;

    for (BlockState dependent : dependents)
      if (checkForDependentDrop(dependent.getBlock())) return true;
    ...
    if (!CreeperConfig.getBool(CfgVal.OVERWRITE_BLOCKS) && !isEmpty(type)) {
      ...
      if (b == null)
        throw new IllegalArgumentException(
            "Null block for: " + block.getState().getType());
      b.drop(true);
      b.remove();
    }
    return false;
  ...
  public void remove() {
    ...
    for (BlockState dependent : dependents) dependent.getBlock().setType(Material.AIR, false);
  }

  @Override
  void record(Collection<ShortLocation> checked) {
    super.record(checked);
    for (BlockState dependent : dependents) checked.add(new ShortLocation(dependent.getLocation()));
  }
}
