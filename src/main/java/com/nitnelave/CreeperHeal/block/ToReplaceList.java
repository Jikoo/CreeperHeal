...
abstract class ToReplaceList {
  ...
  /**
   * Replace the blocks that should be immediately replaced after an explosion, in a task run one
   * tick later.
   */
  static void replaceProtected() {
    Bukkit.getScheduler()
        .scheduleSyncDelayedTask(CreeperHeal.getInstance(), ToReplaceList::replaceAll);
  }
  ...
  private static void replaceAll() {
    for (Replaceable replaceable : toReplace.values()) replaceable.replace(true);
    toReplace.clear();
  }
}
